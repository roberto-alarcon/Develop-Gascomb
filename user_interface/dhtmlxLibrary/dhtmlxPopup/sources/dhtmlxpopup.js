...
function dhtmlXPopup(conf) {
	...
	this._setPos = function(p, state) {
		...
		if (mode == "top" || mode == "bottom") {
			...
			if (mode == "top" && y-this.p.offsetHeight < ty && !state) {
				this._setPos(p, "bottom");
				return;
			}
			
			if (!state && mode == "bottom" && y+h+this.p.offsetHeight > ty2) {
				this._setPos(p, "top");
				return;
			}
		...
		if (mode == "left" || mode == "right") {
			...
			if (mode == "left" && x-this.p.offsetWidth < tx && !state) {
				this._setPos(p, "right");
				return;
			}
			
			if (!state && mode == "right" && x+w+this.p.offsetWidth > tx2) {
				this._setPos(p, "left");
				return;
			}
	...
	// IE6 Display fix
	this._IEDisp = (_isIE && navigator.userAgent.search("MSIE 6.0")>=0);
