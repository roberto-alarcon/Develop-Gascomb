...
<html>
		...
		<script>
			...
			//$(document).bind("pageinit", function() {
				...
				$( '#btnGuardar').on( "click", function(event, ui) {
					...
					var folio_id = $("#folio_id").val();
					var employee_id=$("#employee_id").val();	
					var engine = $("#engine").val();
						
						
					if(folio_id == ""  || folio_id == "null"){
						alert("Es importante agregar el folio del vehiculo");
						$("#folio_id").focus();
						throw ""
					}
					...
					if(employee_id == ""  || employee_id == "null"){
						alert("Es importante seleccionar a la persona que solicita el producto");
						$("#employee_id").focus();
						throw ""
					}
					...
					if(productos.length !== 0){
						...
						$("#titleDialog").html("Ingrese su password personal");
						$("#messageDialog").html('<label for="folio_id">Password:</label> <input type="password" name="requisition_pwd" id="requisition_pwd" value=""  />');
						$.mobile.changePage('#windowDialog', 'pop', true, true);		
											
						
						$( '#enviar').on( "click", function(event, ui) {
								var employee_id=$("#employee_id").val();
								var password = $("#requisition_pwd").val();
								$.get("check_requisition_pwd.php",{ employee_id : employee_id, password : password},function(respuesta){														
									if (respuesta == "true") {									
										$.ajax({
											url: "insert_requisition.php?action=add&folio="+folio_id+"&employee_id="+employee_id+"&products="+productoss,
											type: "POST"						
										})
										.done( function ( response ) {
											if(response == "true"){														
												...
												location.reload();			
											}else if(response == "errorfolio"){
												alert("El Folio ingresado no existe en la base de datos, favor de ingresar uno existente");																
												location.reload();
											}else{
										...
										alert("La clave es incorrecta ó el empleado no tiene permisos para realizar solicitudes de materials");										
									}
								});
						...
						alert("Escriba el producto que busca y seleccionelo de la lista");										
					}
	...
	<body>
		...
		<div data-role="header">
			<h1>  Requisiciones <?php echo $id; ?></h1>
		...
		<div data-role="content">
			...
			<div class="ui-grid-solo">
				<div class="ui-block-a">
					<div data-role="fieldcontain">
										<label for="folio_id"><strong>FOLIO:</strong></label>
										<input type="text" name="folio_id" id="folio_id" value=""  />
					</div>
				</div>
			</div>	
			<div class="ui-grid-solo">				
				...
				<div class="ui-block-a">
					<div data-role="fieldcontain">
						<label for="employee_id" class="select"><strong>Persona que solicita:</strong></label>
						<select name="employee_id" id="employee_id" data-mini="true">
						<?php foreach($mecanics as $mecanic){ echo '<option value='.$mecanic["employee_id"].'>'.utf8_encode($mecanic["name"]." ".$mecanic["last_name"]).'</option>';} ?>
						</select>
					</div>
				</div>
			</div>	
				...
				<ul class="product" id="formContent_0" data-role='listview' data-theme="c" data-inset='true' style="width:100%; margin:10px auto;">
						<li data-role="list-divider">
								<h1>Producto 1:</h1>
						</li>						
						<li data-role="fieldcontain">
							Escriba el producto que busca y seleccionelo de la lista:							
							<div data-role="fieldcontain">							
				...
				<ul class="product" id="formContent_1" data-role='listview' data-inset='true' style="display:none;width:100%; margin:10px auto;">
					<li data-role="list-divider">
								<h1>Producto 2:</h1>
					</li>
					<li data-role="fieldcontain">
				...
				<ul class="product" id="formContent_2" data-role='listview' data-inset='true' style="display:none;width:100%; margin:10px auto;">
					<li data-role="list-divider">
								<h1>Producto 3:</h1>
					</li>
					<li data-role="fieldcontain">
				...
				<ul class="product" id="formContent_3" data-role='listview' data-inset='true' style="display:none;width:100%; margin:10px auto;">
					<li data-role="list-divider">
								<h1>Producto 4:</h1>
					</li>
					<li data-role="fieldcontain">
				...
				<ul class="product" id="formContent_4" data-role='listview' data-inset='true' style="display:none;width:100%; margin:10px auto;">
					<li data-role="list-divider">
								<h1>Producto 5:</h1>
					</li>
					<li data-role="fieldcontain">
				...
				<ul class="product" id="formContent_5" data-role='listview' data-inset='true' style="display:none;width:100%; margin:10px auto;">
					<li data-role="list-divider">
								<h1>Producto 6:</h1>
					</li>
					<li data-role="fieldcontain">
				...
				<ul class="product" id="formContent_6" data-role='listview' data-inset='true' style="display:none;width:100%; margin:10px auto;">
					<li data-role="list-divider">
								<h1>Producto 7:</h1>
					</li>
					<li data-role="fieldcontain">
				...
				<ul class="product" id="formContent_7" data-role='listview' data-inset='true' style="display:none;width:100%; margin:10px auto;">
					<li data-role="list-divider">
								<h1>Producto 8:</h1>
					</li>
					<li data-role="fieldcontain">
				...
				<ul class="product" id="formContent_8" data-role='listview' data-inset='true' style="display:none;width:100%; margin:10px auto;">
					<li data-role="list-divider">
								<h1>Producto 9:</h1>
					</li>
					<li data-role="fieldcontain">
				...
				<ul class="product" id="formContent_9" data-role='listview' data-inset='true' style="display:none;width:100%; margin:10px auto;">
					<li data-role="list-divider">
								<h1>Producto 10:</h1>
					</li>
					<li data-role="fieldcontain">
			...
			<fieldset class="ui-grid-a" style="width:98%; margin:10px auto 0px;">			
				...
				<div class="ui-block-b"><button type="button" id="btnGuardar" data-theme="e" data-icon="check">Enviar</button></div>
			</fieldset>		
		</div>
		<?php  include "footer.php";?>
		</div>		
		<!-- Aqui nuestro dialog con el mensaje de error  -->
		<section id="windowDialog" data-role="dialog">
			<header data-role="header">
				<h1 id="titleDialog">Respuesta</h1>
			</header>
			<article data-role="content">
				<div id="messageDialog">
					<p>Usuario o contraseÃ±a no valida</p>
				</div>
				<a href="#" data-role="button" data-theme="b" id="enviar">Enviar</a>
			</article>
		</section>
