...
if($xml){
	if($models && $xml){
		...
		foreach ($models as $clave => $valor) {							
						echo "<item value='".$valor["support_models_vehicular_id"]."' label='".utf8_encode($valor["model"])." ".utf8_encode($valor["type"])."'/>";
	...
	if($models){
		...
		foreach ($models as $clave => $valor) {							
						echo "<option value='".$valor["support_models_vehicular_id"]."' label='".utf8_encode($valor["model"])."'>".utf8_encode($valor["model"])."</option>";
