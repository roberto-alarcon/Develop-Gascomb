...
Class Quote_Process{
    ...
    public function uploadImage(){
        ...
        // set image path 
        $dir_dest = PATH_MULTIMEDIA_STOCK.$_POST['general_requisicion_id'];
        $dir_pics = URL_MULTIMEDIA_STOCK.$_POST['general_requisicion_id'];
	...
	*/
	public function sendMail($guid , $request_quote_id , $time_set){
		...
		$suppliers = $this->getQuoteSuppliers($request_quote_id);
		
		print_r($suppliers);
		
		foreach( $suppliers as $id_supplier => $concentrado ){
		
			$url = URL_FORM_EMAIL.'?token='.$guid.'&item_supplier='.$id_supplier;
			$anchor_text = $url;
